...
英语课程详情内新增“英语对话练习”：自我介绍、餐厅点单、日常聊天，支持三档难度、文字对话、浏览器语音输入、英语朗读、练后反馈和最近练习记录。

对话复用服务端 DeepSeek / Ollama 配置，持久化到 Academy PostgreSQL。上线前必须应用 `mini-app/postgres/0026_english_conversations.sql`；不修改现有课程完成、积分或能力认证规则。语音识别依赖设备和浏览器支持，不支持时可输入文字；录音不保存到 Academy，也不据此评分发音。

接口、用量边界、验证结果及上线步骤见 [英语对话接入说明](./docs/ENGLISH_CONVERSATION.md)。
...
- 设备/IP 风险评分与邀请奖励人工撤销
- 独立语音服务、可审核录音与真人 10 分钟对话证据（当前浏览器朗读与语音转写不替代这些证据）
- Day 0 / 21 / 30 / 60 能力测评
- 后台人工抽查与反馈工单
