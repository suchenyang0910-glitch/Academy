...
- 课程目录与选课：必选 1 门主课程，其他课程可作为选修且不限数量；主课进入每日监督线，选修进入加学入口，并支持独立 enrollment day。
- 多语言课程内容：界面已支持 zh-Hans / vi / km / th；课程正文必须通过 course_localizations / lesson_localizations 发布审核版，未审核语言显示中文 fallback 与语言提示。
