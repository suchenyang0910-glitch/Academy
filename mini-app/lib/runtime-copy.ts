...
export function courseRuntimeCopy(locale: AppLocale) {
  return {
    "zh-Hans": {
      ...
      minOneCourse: "至少选择 1 门主课程",
      saveFailed: "保存失败",
      cancel: "取消",
      selectedCount: (count: number) =>
        count > 1 ? `主课 1 门 · 选修 ${count - 1} 门` : `已选 ${count} 门主课`,
      dailyMinutes: (minutes: number) => `主课每天约 ${minutes} 分钟，选修按需加学`,
      primaryCourseLabel: "主课程",
      electiveCourseLabel: "可选修",
      setPrimaryCourse: "设为主课",
      savingPlan: "正在安排…",
      ...
      viewCourse: "查看课程 →",
      chooseCourse: "去选择课程",
      courseNotSelected: "这门课尚未加入你的学习计划。你可以先查看它的学习方向，再决定是否选修。",
      electiveWaiting: "这门选修课已加入计划。完成当前主线后，系统会按解锁规则提供本课的加学入口。",
      backCourses: "‹ 返回课程",
      mainline60: "60 天主线",
    ...
    vi: {
      ...
      minOneCourse: "Chọn ít nhất 1 khóa chính",
      saveFailed: "Lưu thất bại",
      cancel: "Hủy",
      selectedCount: (count: number) =>
        count > 1 ? `1 khóa chính · ${count - 1} khóa tự chọn` : `Đã chọn ${count} khóa chính`,
      dailyMinutes: (minutes: number) => `Khóa chính khoảng ${minutes} phút/ngày; tự chọn học thêm khi cần`,
      primaryCourseLabel: "Khóa chính",
      electiveCourseLabel: "Tự chọn",
      setPrimaryCourse: "Đặt làm chính",
      ...
      courseNotSelected: "Khóa này chưa có trong kế hoạch học của bạn. Hãy xem hướng học trước rồi quyết định có học tự chọn hay không.",
      electiveWaiting: "Khóa tự chọn này đã có trong kế hoạch. Sau khi hoàn thành bài chính hiện tại, hệ thống sẽ mở lối học thêm theo quy tắc mở khóa.",
    ...
    km: {
      ...
      minOneCourse: "ត្រូវជ្រើសយ៉ាងហោចណាស់ 1 វគ្គសំខាន់",
      saveFailed: "រក្សាទុកបរាជ័យ",
      cancel: "បោះបង់",
      selectedCount: (count: number) =>
        count > 1 ? `វគ្គសំខាន់ 1 · ជម្រើស ${count - 1}` : `បានជ្រើសវគ្គសំខាន់ ${count}`,
      dailyMinutes: (minutes: number) => `វគ្គសំខាន់ប្រហែល ${minutes} នាទី/ថ្ងៃ; ជម្រើសរៀនបន្ថែមតាមតម្រូវការ`,
      primaryCourseLabel: "វគ្គសំខាន់",
      electiveCourseLabel: "ជម្រើស",
      setPrimaryCourse: "ដាក់ជាសំខាន់",
      ...
      viewCourse: "មើលវគ្គ →",
      chooseCourse: "ជ្រើសវគ្គ",
      courseNotSelected: "វគ្គនេះមិនទាន់មានក្នុងផែនការសិក្សារបស់អ្នកទេ។ អាចមើលទិសដៅសិក្សាសិន រួចសម្រេចថាតើចង់ជ្រើសជាជម្រើសឬអត់។",
      electiveWaiting: "វគ្គជម្រើសនេះបានបញ្ចូលក្នុងផែនការហើយ។ បន្ទាប់ពីបញ្ចប់មេរៀនសំខាន់បច្ចុប្បន្ន ប្រព័ន្ធនឹងបើកច្រករៀនបន្ថែមតាមច្បាប់ដោះសោ។",
    ...
    th: {
      ...
      minOneCourse: "ต้องเลือกคอร์สหลักอย่างน้อย 1 คอร์ส",
      saveFailed: "บันทึกไม่สำเร็จ",
      cancel: "ยกเลิก",
      selectedCount: (count: number) =>
        count > 1 ? `คอร์สหลัก 1 · เสริม ${count - 1} คอร์ส` : `เลือกคอร์สหลัก ${count} คอร์ส`,
      dailyMinutes: (minutes: number) => `คอร์สหลักประมาณ ${minutes} นาที/วัน; คอร์สเสริมเรียนเพิ่มได้ตามต้องการ`,
      primaryCourseLabel: "คอร์สหลัก",
      electiveCourseLabel: "คอร์สเสริม",
      setPrimaryCourse: "ตั้งเป็นหลัก",
      ...
      viewCourse: "ดูคอร์ส →",
      chooseCourse: "เลือกคอร์ส",
      courseNotSelected: "คอร์สนี้ยังไม่อยู่ในแผนการเรียนของคุณ ดูแนวทางการเรียนก่อน แล้วค่อยตัดสินใจว่าจะเลือกเป็นคอร์สเสริมหรือไม่",
      electiveWaiting: "คอร์สเสริมนี้อยู่ในแผนแล้ว เมื่อทำบทหลักปัจจุบันเสร็จ ระบบจะเปิดทางเรียนเพิ่มตามกติกาการปลดล็อก",
...
export function todayRuntimeCopy(locale: AppLocale) {
  return {
    "zh-Hans": {
      ...
      chooseCoursesTitle: "选择 1 门主课，可加选任意选修",
      chooseCoursesSubtitle: "主课每天 15–20 分钟；选修完成主课后再加学",
    ...
    vi: {
      ...
      chooseCoursesTitle: "Chọn 1 khóa chính, thêm tự chọn tùy ý",
      chooseCoursesSubtitle: "Khóa chính 15–20 phút/ngày; tự chọn mở sau bài chính",
    ...
    km: {
      ...
      chooseCoursesTitle: "ជ្រើសវគ្គសំខាន់ 1 ហើយបន្ថែមជម្រើសបាន",
      chooseCoursesSubtitle: "វគ្គសំខាន់ 15–20 នាទី/ថ្ងៃ; ជម្រើសបើកបន្ទាប់ពីបញ្ចប់មេរៀនសំខាន់",
    ...
    th: {
      ...
      chooseCoursesTitle: "เลือกคอร์สหลัก 1 คอร์ส และเพิ่มคอร์สเสริมได้",
      chooseCoursesSubtitle: "คอร์สหลัก 15–20 นาที/วัน; คอร์สเสริมเปิดหลังทำบทหลัก",
...
export function profileRuntimeCopy(locale: AppLocale) {
  return {
    "zh-Hans": {
      ...
      courseCount: (count: number) => `${count} 门`,
    ...
    vi: {
      ...
      courseCount: (count: number) => `${count} khóa`,
    ...
    km: {
      ...
      courseCount: (count: number) => `${count} វគ្គ`,
    ...
    th: {
      ...
      courseCount: (count: number) => `${count} คอร์ส`,
